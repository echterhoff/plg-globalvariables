...
/**
 * Options:
 * 		see defaults $.JBlockrepeat.defaults,
 * Options can be set through "data" attribute of the JBlockrepeat container (see example markup)
 *
 * Events:
 * 		$('input.form-field-blockrepeat')
 * 		.on('weready', function(e){
 * 			// fires when JBlockrepeat initialized
 * 		})
 ...
 * 		/>
 *
 * data-blockrepeat-element="table tbody tr" - means that <tr> inside <tbody> will be repeatable
 */
...
(function($) {
    ...
    $.JBlockrepeat = function(input, options) {
        // To avoid scope issues,
        var self = this;

        //direct call
        if (!self || self === window) {
            return new $.JBlockrepeat(input, options);
        }

        self.$input = $(input);

        // check if alredy exist
        if (self.$input.data("JBlockrepeat")) {
            return self;
        }

        // Add a reverse reference to the DOM object
        self.$input.data("JBlockrepeat", self);

        // method initialize
        self.init = function() {
            // merge options
            self.options = $.extend({}, $.JBlockrepeat.defaults, options);
            ...
            self.$rowsContainer = self.$container.find(self.options.blockrepeatElement).parent();
            ...
            self.$container.on('click', self.options.btAdd, function(e) {
                e.preventDefault();
                var after = $(this).parents(self.options.blockrepeatElement);
                if (!after.length) {
                    after = null;
                }
                self.addRow(after);
            ...
            self.$container.on('click', self.options.btRemove, function(e) {
                e.preventDefault();
                var row = $(this).parents(self.options.blockrepeatElement);
                self.removeRow(row);
            });
        ...
        self.prepareTemplate = function() {
            //find available
            var $rows = self.$container.find(self.options.blockrepeatElement);
            var $row = $($rows.get(0));
        ...
        self.prepareModal = function() {
            ...
            self.$modalWindow = modalEl.modal({
                show: false,
                backdrop: 'static'
            });
        ...
        self.addRow = function(after, valueKey) {
            ...
            var count = self.$container.find(self.options.blockrepeatElement).length;
    ...
    $.fn.JBlockrepeat = function(options) {
    ...
    $(window).on('load', function() {
        $('input.form-field-blockrepeat').JBlockrepeat();
    })
